...
void IntHandlerDrvAdcCommonData(void) {
    ...
    if (true == DRV_ADC_SamplesAvailable(ADC_TEMPERATURE_SENSOR_1)) {
        data = (uint16_t)DRV_ADC_SamplesRead(ADC_TEMPERATURE_SENSOR_1);
        ADC_UpdateData(ADC_TEMPERATURE_SENSOR_1, data);
    }
    if (true == DRV_ADC_SamplesAvailable(ADC_TEMPERATURE_SENSOR_2)) {
        data = (uint16_t)DRV_ADC_SamplesRead(ADC_TEMPERATURE_SENSOR_2);
        ADC_UpdateData(ADC_TEMPERATURE_SENSOR_2, data);
    }
    if (true == DRV_ADC_SamplesAvailable(ADC_TEMPERATURE_SENSOR_3)) {
        data = (uint16_t)DRV_ADC_SamplesRead(ADC_TEMPERATURE_SENSOR_3);
        ADC_UpdateData(ADC_TEMPERATURE_SENSOR_3, data);
    }

...
static void drawBackground(jfloLabelWidget* lbl);
static void drawMultiLineString(jfloLabelWidget* lbl);
static void waitString(jfloLabelWidget* lbl);
static void drawBorder(jfloLabelWidget* lbl);



static void nextState(jfloLabelWidget* lbl)
{
...
static void drawBackground(jfloLabelWidget* lbl)
{
    GFX_Rect layerRect = laUtils_WidgetLayerRect((laWidget*)lbl);
    laLayer* layer = laUtils_GetLayer((laWidget*)lbl);

    const GFX_Rect* rect = &layerRect;
    GFX_Color clr = lbl->widget.scheme->background;
    uint32_t radius = lbl->widget.cornerRadius;
    const GFX_Rect* dmgRect = &layer->clippedDrawingRect;  

    GFX_Rect clipRect;
    GFX_Rect drawRect;

    GFX_Set(GFXF_DRAW_COLOR, lbl->widget.scheme->background);
